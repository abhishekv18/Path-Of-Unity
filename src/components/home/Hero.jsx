...
const Hero = () => {
  ...
  // Removed the dagger symbol and enhanced the collection
  const interfaithSymbols = ['✝️', '☪️', '🕎', '☸️', '🔯', '☮️', '⭐', '🌟', '💫', '🕉️', '🙏', '🌸', '💎', '🌺', '🕊️'];
  ...
  useEffect(() => {
    ...
    // Rotate quotes with smoother transition
    const quoteInterval = setInterval(() => {
      setCurrentQuote((prev) => (prev + 1) % interfaithQuotes.length);
    }, 5000);

    // Enhanced mouse move handler for immersive parallax
    const handleMouseMove = (e) => {
      setMousePosition({
        x: (e.clientX / window.innerWidth) * 100,
        y: (e.clientY / window.innerHeight) * 100
  ...
  return (
    <div className="relative min-h-screen overflow-hidden bg-gradient-to-br from-yellow-50 via-amber-25 to-orange-50">
      ...
      <div className="absolute inset-0 w-full h-full">
        <div className="absolute inset-0 w-full h-full bg-black">
          <div className="w-full h-full bg-gradient-to-br from-yellow-400/85 via-amber-300/70 to-orange-300/80 opacity-95"></div>
          {/* Layered golden overlays for depth */}
          <div className="absolute inset-0 bg-gradient-to-br from-yellow-400/45 via-amber-300/35 to-orange-300/45"></div>
          <div className="absolute inset-0 bg-gradient-to-tr from-blue-400/15 via-transparent to-blue-500/20"></div>
        </div>
        
        {/* Enhanced spiritual overlay with texture */}
        <div className="absolute inset-0 bg-gradient-to-br from-yellow-50/90 via-amber-50/85 to-orange-50/90"></div>
        <div className="absolute inset-0 bg-gradient-to-t from-amber-100/40 via-transparent to-blue-50/30"></div>
      </div>
      ...
      <div className="absolute inset-0">
        ...
        <div className="absolute inset-0 opacity-25">
          <div 
            className="absolute inset-0 bg-gradient-to-r from-blue-500/20 via-yellow-400/25 to-purple-500/18"
            style={{
              transform: `translate(${mousePosition.x * 0.03}px, ${mousePosition.y * 0.03}px)`,
              transition: 'transform 1s ease-out',
              animation: 'enhancedWave 18s ease-in-out infinite'
            }}
          ></div>
          <div 
            className="absolute inset-0 bg-gradient-to-l from-amber-400/15 via-blue-400/20 to-yellow-500/15"
            style={{
              transform: `translate(${mousePosition.x * -0.02}px, ${mousePosition.y * -0.02}px)`,
              transition: 'transform 1.2s ease-out',
              animation: 'enhancedWave 22s ease-in-out infinite reverse'
            }}
          ></div>
        </div>
        
        {/* Interactive radial gradient with smoother transitions */}
        <div 
          className="absolute inset-0 opacity-35 transition-opacity duration-1000"
          style={{
            background: `radial-gradient(circle at ${mousePosition.x}% ${mousePosition.y}%, rgba(251, 191, 36, 0.25) 0%, rgba(59, 130, 246, 0.1) 40%, transparent 70%)`
          }}
        ></div>

        {/* Enhanced Divine light rays with more complexity */}
        <div className="absolute top-0 left-1/2 transform -translate-x-1/2 w-[600px] h-[600px] opacity-18">
          <div className="absolute inset-0 bg-gradient-to-b from-yellow-400/30 via-transparent to-transparent transform rotate-15" style={{ animation: 'enhancedPulse 12s ease-in-out infinite' }}></div>
          <div className="absolute inset-0 bg-gradient-to-b from-amber-400/25 via-transparent to-transparent transform -rotate-15" style={{ animation: 'enhancedPulse 12s ease-in-out infinite', animationDelay: '4s' }}></div>
          <div className="absolute inset-0 bg-gradient-to-b from-blue-400/20 via-transparent to-transparent" style={{ animation: 'enhancedPulse 12s ease-in-out infinite', animationDelay: '8s' }}></div>
          <div className="absolute inset-0 bg-gradient-to-b from-purple-400/15 via-transparent to-transparent transform rotate-30" style={{ animation: 'enhancedPulse 12s ease-in-out infinite', animationDelay: '2s' }}></div>
        </div>
      </div>
      ...
      <div className="absolute inset-0 pointer-events-none">
        {particles.map((particle) => (
          <div
            key={particle.id}
            className="absolute opacity-50 hover:opacity-80 transition-all duration-1000"
            style={{
              left: `${particle.x}%`,
              top: `${particle.y}%`,
              fontSize: `${particle.size * 1.8}rem`,
              animation: `enhancedFloat ${particle.duration}s ease-in-out infinite`,
              animationDelay: `${particle.delay}s`,
              transform: `translate(${mousePosition.x * 0.015}px, ${mousePosition.y * 0.015}px) rotate(${particle.rotation}deg)`,
              color: particle.id % 4 === 0 ? '#1e40af' : 
                     particle.id % 4 === 1 ? '#d97706' : 
                     particle.id % 4 === 2 ? '#7c2d12' : '#9333ea',
      ...
      {/* Enhanced Sacred Geometry with More Elements */}
      <div className="absolute inset-0 opacity-10">
        <div 
          className="absolute top-1/4 left-1/4 w-96 h-96 rounded-full border-2 border-blue-400/25"
          style={{ 
            animation: 'slowSpin 60s linear infinite',
            transform: `translate(${mousePosition.x * 0.04}px, ${mousePosition.y * 0.04}px)`
          }}
        >
          <div className="absolute inset-8 rounded-full border border-amber-400/20" style={{ animation: 'slowSpin 40s linear infinite reverse' }}></div>
          <div className="absolute inset-16 rounded-full border border-blue-300/15" style={{ animation: 'slowSpin 30s linear infinite' }}></div>
          <div className="absolute inset-24 rounded-full border border-purple-300/12" style={{ animation: 'slowSpin 20s linear infinite reverse' }}></div>
        </div>
        ...
        >
          <div className="absolute top-1/2 left-0 right-0 h-0.5 bg-blue-500/25 transform -translate-y-1/2"></div>
          <div className="absolute left-1/2 top-0 bottom-0 w-0.5 bg-blue-500/25 transform -translate-x-1/2"></div>
          <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-12 h-12 rounded-full bg-amber-400/30 border-2 border-blue-400/20"></div>
        </div>
        
        <div 
          className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] rounded-full border-2 border-amber-300/20"
          style={{ animation: 'enhancedExpand 15s ease-in-out infinite' }}
        ></div>
        
        {/* Additional geometric elements */}
        <div 
          className="absolute top-3/4 left-1/4 w-32 h-32 transform rotate-45"
          style={{ animation: 'slowSpin 45s linear infinite' }}
        >
          <div className="w-full h-full border border-purple-400/15 rounded-lg"></div>
        </div>
      </div>
      ...
      <div className="relative z-10 flex flex-col items-center justify-center min-h-screen px-4 text-center pt-20">
        ...
        {/* Enhanced ILA Logo */}
        <div className={`mb-16 transform transition-all duration-3000 ${isVisible ? 'translate-y-0 opacity-100' : 'translate-y-20 opacity-0'}`}>
          <div className="relative group">
            <div className="w-64 h-64 mx-auto mb-10 rounded-full bg-gradient-to-r from-blue-600 via-amber-400 to-blue-700 p-3 shadow-2xl shadow-amber-500/40 hover:shadow-amber-500/60 transition-all duration-700 hover:scale-105">
              <div className="w-full h-full rounded-full bg-gradient-to-br from-yellow-50/99 to-amber-50/99 backdrop-blur-xl flex items-center justify-center relative overflow-hidden border-2 border-amber-200/50">
                {/* Enhanced inner glow effect */}
                <div className="absolute inset-0 rounded-full bg-gradient-to-r from-blue-400/12 via-amber-400/18 to-blue-400/12" style={{ animation: 'enhancedPulse 6s ease-in-out infinite' }}></div>
                <div className="absolute inset-4 rounded-full bg-gradient-to-br from-yellow-300/10 to-amber-300/10" style={{ animation: 'enhancedPulse 8s ease-in-out infinite reverse' }}></div>
                
                {/* International Laity Association Logo */}
                <div className="relative z-10 w-full h-full flex flex-col items-center justify-center p-6">
                  {/* Religious Symbols arranged around central book */}
                  <div className="relative w-36 h-36 mb-3">
                    {/* Enhanced Central Open Book */}
                    <div className="absolute bottom-3 left-1/2 transform -translate-x-1/2">
                      <div className="w-18 h-12 relative">
                        <div className="absolute inset-0 bg-gradient-to-br from-blue-800 to-blue-900 rounded-md transform -rotate-3 shadow-lg"></div>
                        <div className="absolute inset-0 bg-gradient-to-br from-blue-700 to-blue-800 rounded-md transform rotate-3 shadow-lg"></div>
                        <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-0.5 h-10 bg-gradient-to-b from-amber-300 to-amber-400 shadow-sm"></div>
                      </div>
                    </div>
                    
                    {/* Enhanced Interfaith Symbols */}
                    <div className="absolute top-1 left-3 text-blue-700 text-2xl" style={{ animation: 'gentleFloat 7s ease-in-out infinite' }}>🕉️</div>
                    
                    <div className="absolute top-1 left-1/2 transform -translate-x-1/2">
                      <div className="w-2 h-10 bg-gradient-to-b from-blue-700 to-blue-900 rounded-full shadow-lg"></div>
                      <div className="absolute top-3 left-1/2 transform -translate-x-1/2 w-7 h-2 bg-gradient-to-r from-blue-700 to-blue-900 rounded-full shadow-lg"></div>
                    </div>
                    
                    <div className="absolute top-1 right-3 text-blue-700 text-2xl" style={{ animation: 'gentleFloat 7s ease-in-out infinite', animationDelay: '1s' }}>☪️</div>
                    
                    <div className="absolute bottom-1 left-1 text-blue-700 text-xl" style={{ animation: 'gentleFloat 7s ease-in-out infinite', animationDelay: '2s' }}>🕎</div>
                    
                    <div className="absolute bottom-1 right-1 text-blue-700 text-xl" style={{ animation: 'gentleFloat 7s ease-in-out infinite', animationDelay: '3s' }}>☸️</div>
                  </div>
                ...
                {/* Enhanced orbiting light effects */}
                <div className="absolute inset-0">
                  <div className="absolute top-6 left-1/2 transform -translate-x-1/2 w-3 h-3 bg-amber-400 rounded-full blur-sm shadow-lg" style={{ animation: 'enhancedOrbit 18s linear infinite' }}></div>
                  <div className="absolute bottom-6 left-1/2 transform -translate-x-1/2 w-3 h-3 bg-blue-400 rounded-full blur-sm shadow-lg" style={{ animation: 'enhancedOrbit 18s linear infinite reverse' }}></div>
                  <div className="absolute left-6 top-1/2 transform -translate-y-1/2 w-2.5 h-2.5 bg-amber-500 rounded-full blur-sm shadow-lg" style={{ animation: 'enhancedOrbit 22s linear infinite', animationDelay: '5s' }}></div>
                  <div className="absolute right-6 top-1/2 transform -translate-y-1/2 w-2.5 h-2.5 bg-blue-500 rounded-full blur-sm shadow-lg" style={{ animation: 'enhancedOrbit 22s linear infinite reverse', animationDelay: '5s' }}></div>
                </div>
              </div>
            </div>
            
            {/* Enhanced Floating icons around logo */}
            <div className="absolute -top-8 -right-8" style={{ animation: 'enhancedFloat 6s ease-in-out infinite' }}>
              <Star className="w-12 h-12 text-amber-500 drop-shadow-2xl filter hover:scale-110 transition-transform duration-300" />
            </div>
            <div className="absolute -bottom-8 -left-8" style={{ animation: 'enhancedFloat 6s ease-in-out infinite', animationDelay: '2s' }}>
              <Heart className="w-12 h-12 text-blue-600 drop-shadow-2xl filter hover:scale-110 transition-transform duration-300" />
            </div>
            <div className="absolute top-1/2 -right-12 transform -translate-y-1/2" style={{ animation: 'enhancedFloat 7s ease-in-out infinite', animationDelay: '3.5s' }}>
              <div className="text-4xl drop-shadow-xl hover:scale-110 transition-transform duration-300">☮️</div>
            </div>
            <div className="absolute top-1/2 -left-12 transform -translate-y-1/2" style={{ animation: 'enhancedFloat 7s ease-in-out infinite', animationDelay: '5s' }}>
              <div className="text-4xl drop-shadow-xl hover:scale-110 transition-transform duration-300">🙏</div>
            </div>
          </div>
        </div>

        {/* Enhanced Main Heading with Better Typography */}
        <h1 className={`text-7xl md:text-9xl lg:text-10xl font-bold mb-10 transform transition-all duration-3500 delay-700 ${isVisible ? 'translate-y-0 opacity-100' : 'translate-y-20 opacity-0'}`}>
          <span className="relative">
            <span className="bg-gradient-to-r from-blue-600 via-blue-700 to-yellow-600 bg-clip-text text-transparent font-black tracking-tighter hover:tracking-tight transition-all duration-700">
              Path of Faith
            </span>
            <span className="absolute inset-0 bg-gradient-to-r from-blue-700 via-blue-800 to-yellow-600 bg-clip-text text-transparent blur-sm opacity-20" style={{ animation: 'enhancedPulse 5s ease-in-out infinite' }}>
              Path of Faith
            </span>
          </span>
        </h1>
        ...
        <p className={`text-2xl md:text-4xl lg:text-5xl text-gray-800 mb-20 max-w-7xl font-light leading-relaxed transform transition-all duration-3500 delay-1000 ${isVisible ? 'translate-y-0 opacity-100' : 'translate-y-20 opacity-0'}`}>
          <span className="bg-gradient-to-r from-blue-800 via-blue-700 to-amber-800 bg-clip-text text-transparent tracking-wide">
            Walking in God's Love, Growing in His Grace
          </span>
        </p>

        {/* Enhanced Rotating Quotes with Better Design */}
        <div className={`mb-24 h-32 flex items-center justify-center transform transition-all duration-3500 delay-1300 ${isVisible ? 'translate-y-0 opacity-100' : 'translate-y-20 opacity-0'}`}>
          <div className="text-center max-w-6xl relative group">
            <div className="absolute inset-0 bg-gradient-to-br from-yellow-50/98 to-amber-50/98 backdrop-blur-lg rounded-3xl border-2 border-amber-200/60 shadow-2xl group-hover:shadow-3xl transition-all duration-700"></div>
            <div className="absolute inset-0 bg-gradient-to-br from-blue-50/30 to-purple-50/20 rounded-3xl opacity-0 group-hover:opacity-100 transition-opacity duration-700"></div>
            <div className="relative p-12">
              <p className="text-xl md:text-3xl lg:text-4xl text-gray-800 italic mb-6 transition-all duration-1500 leading-relaxed font-light">
                "{interfaithQuotes[currentQuote].text}"
              </p>
              <p className="text-lg md:text-xl text-blue-700 font-bold tracking-wide">
                — {interfaithQuotes[currentQuote].author}
              </p>
            </div>
          </div>
        ...
        >
          <div className="relative p-2 sm:p-4 lg:p-6 rounded-full bg-gradient-to-br from-yellow-50/98 to-amber-50/98 backdrop-blur-lg border-2 border-amber-300/70 group-hover:bg-amber-50/99 transition-all duration-500 shadow-2xl group-hover:shadow-3xl">
      ...
      {/* Enhanced Bottom Gradient */}
      <div className="absolute bottom-0 left-0 right-0 h-56 bg-gradient-to-t from-amber-50 via-yellow-50/70 to-transparent"></div>
      ...
      <style jsx>{`
        ...
        @keyframes enhancedWave {
          ...
          50% { 
            transform: translateX(12px) translateY(18px) rotate(-1deg);
            opacity: 0.22;
          }
          75% { 
            transform: translateX(-15px) translateY(-8px) rotate(-1.5deg);
