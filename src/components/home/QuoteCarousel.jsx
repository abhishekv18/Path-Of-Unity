...
const QuoteCarousel = ({ quotes = [] }) => {
  ...
  const defaultQuotes = [
    {
      ...
      tradition: "Universal",
      category: "Love",
      backgroundColor: "from-blue-50 to-amber-50"
    },
    {
      id: 2,
      text: "Be still and know that I am God",
    ...
    },
    {
      ...
      tradition: "Christian",
      category: "Love",
      backgroundColor: "from-blue-50 to-amber-100"
    },
    {
      id: 5,
      text: "Where there is dharma, there is victory",
      ...
      tradition: "Hindu",
      category: "Wisdom",
      backgroundColor: "from-yellow-100 to-blue-50"
    }
  ];
  ...
  return (
    <section 
      ref={sectionRef}
      className="relative py-16 md:py-24 overflow-hidden bg-gradient-to-br from-white via-gray-50 to-blue-50"
      onMouseEnter={() => setIsPaused(true)}
      onMouseLeave={() => setIsPaused(false)}
    >
      {/* Floating Wisdom Elements */}
      ...
      <div className="absolute inset-0">
        <div 
          className={`w-full h-full transition-all duration-1000 bg-gradient-to-br ${currentQuoteData.backgroundColor || 'from-yellow-100 to-blue-100'}`}
        />
      </div>
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-6">
        ...
        {/* Main Quote Display */}
        <div className={`relative transform transition-all duration-1000 delay-300 ${isVisible ? 'translate-y-0 opacity-100' : 'translate-y-8 opacity-0'}`}>
          <div className="bg-white backdrop-blur-sm rounded-3xl p-8 md:p-12 shadow-2xl border border-blue-100 relative overflow-hidden">
            
            {/* Decorative Quote Marks */}
            <div className="absolute top-4 left-4 opacity-20">
              <Quote className="w-12 h-12 text-yellow-400 transform rotate-180" />
            </div>
            <div className="absolute bottom-4 right-4 opacity-20">
            ...
            <div className="relative text-center">
              ...
              <div className="inline-flex items-center gap-2 mb-6">
                <div className={`px-4 py-2 rounded-full text-sm font-medium ${
                  currentQuoteData.tradition === 'Christian' ? 'bg-blue-100 text-blue-700 border border-blue-200' :
                  currentQuoteData.tradition === 'Hindu' ? 'bg-yellow-100 text-yellow-700 border border-yellow-200' :
                  'bg-gradient-to-r from-blue-100 to-yellow-100 text-blue-700 border border-blue-200'
                }`}>
                  {currentQuoteData.tradition} Wisdom
                </div>
              </div>
              ...
              {/* Quote Text */}
              <blockquote className="font-serif text-2xl md:text-3xl lg:text-4xl font-light text-blue-900 mb-8 leading-relaxed italic">
                "{currentQuoteData.text}"
              </blockquote>
              ...
              <div className="flex justify-center items-center gap-4">
                <button
                  ...
                  className={`p-3 rounded-full transition-all duration-300 ${
                    favorites.includes(currentQuoteData.id)
                      ...
                      : 'bg-white text-blue-700 hover:bg-red-50 hover:text-red-500 border border-blue-200'
                  }`}
                  title="Add to Favorites"
                >
                  <Heart className={`w-5 h-5 ${favorites.includes(currentQuoteData.id) ? 'fill-current' : ''}`} />
                ...
                <button
                  onClick={() => shareQuote(currentQuoteData)}
                  className="p-3 rounded-full bg-white text-blue-700 hover:bg-blue-50 hover:text-blue-600 transition-all duration-300 border border-blue-200"
                  title="Share Quote"
                >
                  <Share2 className="w-5 h-5" />
                </button>
                ...
                <button
                  className="p-3 rounded-full bg-white text-blue-700 hover:bg-yellow-50 hover:text-yellow-600 transition-all duration-300 border border-blue-200"
                  title="Save Quote"
                >
                  <Bookmark className="w-5 h-5" />
                </button>
        ...
        <div className={`flex justify-center items-center mt-12 gap-8 transform transition-all duration-1000 delay-500 ${isVisible ? 'translate-y-0 opacity-100' : 'translate-y-8 opacity-0'}`}>
          ...
          <button
            onClick={prevQuote}
            className="group p-4 bg-white backdrop-blur-sm rounded-full hover:bg-blue-50 transition-all duration-300 hover:scale-110 shadow-lg border border-blue-100"
          >
            <ChevronLeft className="w-6 h-6 text-blue-700 group-hover:text-blue-600 transition-colors duration-300" />
          </button>
          ...
          <div className="flex gap-2">
            {quotesToShow.map((_, index) => (
              <button
                ...
                className={`w-3 h-3 rounded-full transition-all duration-300 ${
                  index === currentQuote 
                    ? 'bg-gradient-to-r from-blue-500 to-yellow-500 scale-125' 
          ...
          <button
            ...
            className="group p-4 bg-white backdrop-blur-sm rounded-full hover:bg-blue-50 transition-all duration-300 hover:scale-110 shadow-lg border border-blue-100"
          >
            <ChevronRight className="w-6 h-6 text-blue-700 group-hover:text-blue-600 transition-colors duration-300" />
          </button>
        </div>
